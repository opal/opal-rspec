...
RSpec.describe 'Opal Specs' do
  context 'as a whole' do
    ...
    before :all do
      @test_output = `rake spec:opal`.force_encoding('UTF-8')
      @test_status = $?
    end
    ...
    it 'has a summary line' do
      ...
      summary_line = test_output.scan(/(\d+) examples, (\d+) failures, (\d+) pending/).first.first
      expect(summary_line).to eq("1 examples, 39 failures, 13 pending")
    end
    ...
    it 'has the expected failures' do
      ...
      actual_failures = test_output.scan(/\d+\) (.*)/).map(&:first)

      expect(actual_failures).to eq(expected_failures)
    end
